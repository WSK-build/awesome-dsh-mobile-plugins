/**
 * Publish `plugins.json` to npm as the `dsh-mobile-plugin-catalog` package.
 *
...
const PKG = 'dsh-mobile-plugin-catalog'
...
function readme(version, entries) {
  return `# dsh-mobile-plugin-catalog

The DeepSeek Harness plugin catalog as a single JSON file — the same
\`plugins.json\` served at <https://wsk-build.github.io/awesome-dsh-mobile-plugins/plugins.json>,
...
\`\`\`js
import catalog from 'dsh-mobile-plugin-catalog/plugins.json' with { type: 'json' }
\`\`\`

- **Entries:** ${entries}
- **Version:** \`${version}\` (\`YYYY.MDD.BUILD\`, published only when the catalog changes)
- **Source:** <https://github.com/WSK-build/awesome-dsh-mobile-plugins>
- **License:** CC0-1.0
...
fs.writeFileSync(path.join(stage, 'package.json'), JSON.stringify({
  ...
  homepage: 'https://wsk-build.github.io/awesome-dsh-mobile-plugins',
  repository: {
    type: 'git',
    url: 'git+https://github.com/WSK-build/awesome-dsh-mobile-plugins.git',
  },
  license: 'CC0-1.0',
